...
서비스 주소: **`https://meetcute.madam-ddu.com`**
...
MEETCUTE_TUNNEL_HOSTNAME=meetcute.madam-ddu.com
```
...
2. DNS 라우팅 (`meetcute.madam-ddu.com` → 터널)
3. `.public_url` 에 `https://meetcute.madam-ddu.com` 기록 → 앱이 감지해 마담뚜에게 새 URL 텔레그램 알림
4. 터널 실행
...
URL 은 **항상 `https://meetcute.madam-ddu.com`** — 안 바뀌므로 마담뚜에게 재공유 불필요.
...
`meetcute.madam-ddu.com` 을 쓰면 루트 `madam-ddu.com` 은 나중에 랜딩/다른 용도로 남겨둘 수 있다. 루트를 직접 쓰고 싶으면 `MEETCUTE_TUNNEL_HOSTNAME=madam-ddu.com` 으로 바꾸면 된다.

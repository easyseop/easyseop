...
### MySQL/PostgreSQL 쓸 거면
환경변수 한 줄:
```bash
export MEETCUTE_DB_URL="mysql+pymysql://root:비번@127.0.0.1:3306/meetcute?charset=utf8mb4"
```
서버가 첫 부팅 시 `meetcute` DB와 모든 테이블을 자동 생성합니다.

### 🌱 시드 스크립트
```bash
python -m app.seed           # 빈 DB일 때만 채움
...
| 4.7 | 텔레그램 알림 (선택) | ✅ |
| 4.8 | 모바일 UI · 외부 노출(cloudflared) · PUBLIC 모드 안전 가드 | ✅ |
| 4.9 | 마담뚜 임시 대화방 (72h 휘발 · 텔레그램 핑 · 책임자 열람) | ✅ |
| 5 | 사진 자동 회전(EXIF) · 썸네일 · 드래그 정렬 | ⬜ |
| 6 | 통계 페이지 (커플 성사율·요청 수락률·결과 분포·월별 추이·마담뚜별 성과) | ✅ |
